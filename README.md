...
> 💡 **演示模式 vs 生产模式**：
> - **演示模式**（默认）：使用 mock LLM，不需要 API 密钥即可体验核心流程，AI 功能展示预设响应，数据 CRUD 真实保存
...
**登录后即可体验**：
1. 岗位探索：导入或创建岗位，收藏到岗位库 ✅ 真实保存
2. 简历优化：从岗位探索带入岗位信息，进行 JD 定向优化 ✅ 真实保存（AI 响应为 mock）
3. 面试准备：生成面试题目或开始 AI 面试教练对练 ✅ 真实保存（AI 响应为 mock）
4. 设置中心：查看简历、岗位、面试记录管理，可查看 Agent 配置界面

**如需真实 AI 能力**：切换到生产环境并配置有效的 API Key，参见 docker/README.md
